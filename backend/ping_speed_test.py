...
# ==== REAL PPTP Authentication Test ====
async def test_real_pptp_auth(ip: str, login: str, password: str, timeout: float = 20.0) -> Dict:
    """
    РЕАЛЬНАЯ проверка PPTP авторизации через pppd
    Возвращает success=True ТОЛЬКО если PPTP авторизация успешна
    """
    import subprocess
    import tempfile
    import os
    ...
    node_test_id = f"test_{ip.replace('.', '_')}"
    peer_file = Path(f"/etc/ppp/peers/{node_test_id}")
    log_file = Path(f"/tmp/pptp_auth_test_{node_test_id}.log")
    
    try:
        # Создаем временный peer файл
...
"""
        ...
        success = False
        if 'CHAP authentication succeeded' in log_content or 'authenticated' in log_content.lower():
            if 'local  IP address' in log_content and 'remote IP address' in log_content:
                success = True
        ...
        try:
            ...
            # Убиваем pppd если еще работает
        ...
        if success:
            ...
            return {
                "success": False,
                "message": "PPTP auth FAILED - check credentials or server offline"
            }
