...
async def test_real_pptp_speed(ip: str, login: str, password: str, sample_kb: int = 128, timeout: int = 60) -> Dict:
    """
    РЕАЛЬНЫЙ Speed Test через PPTP туннель
    1. Поднимает PPTP туннель
    2. Ждет PPP interface UP
    3. Измеряет скорость ЧЕРЕЗ туннель (ping через ppp, download через туннель)
    4. Закрывает туннель и очищает
    """
    import subprocess
    import asyncio
    from pathlib import Path
    
    node_test_id = f"speed_test_{ip.replace('.', '_')}"
    peer_file = Path(f"/etc/ppp/peers/{node_test_id}")
...
"""
        ...
        chap_file = Path("/etc/ppp/chap-secrets")
        if chap_file.exists():
        ...
        start_time = time.time()
        process = await asyncio.create_subprocess_exec(
            "pppd", "call", node_test_id, "debug",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        
        # 3. Ждем PPP interface UP (до 20 секунд)
        ppp_iface = None
        for attempt in range(20):
            await asyncio.sleep(1)
            
            # Проверяем есть ли ppp interface
            result = subprocess.run("ip link show | grep 'ppp.*UP'", shell=True, capture_output=True, text=True)
            if 'ppp' in result.stdout and 'UP' in result.stdout:
                # Найти номер интерфейса
                import re
        ...
        if not ppp_iface:
            ...
            return {
                ...
                "upload_mbps": 0.0,
                "ping_ms": 0.0,
                "message": "PPTP tunnel did not come UP"
            }
        
        # 4. Измеряем скорость через туннель
        # Получаем remote IP
        ip_result = subprocess.run(f"ip addr show {ppp_iface}", shell=True, capture_output=True, text=True)
        remote_ip = None
        for line in ip_result.stdout.split('\n'):
            if 'peer' in line:
                import re
                match = re.search(r'peer (\d+\.\d+\.\d+\.\d+)', line)
                if match:
        ...
        if remote_ip:
            # Ping через туннель
            ping_result = subprocess.run(f"ping -c 3 -W 2 {remote_ip}", shell=True, capture_output=True, text=True)
            ping_ms = 0.0
            if 'avg' in ping_result.stdout:
                import re
                match = re.search(r'avg.*?(\d+\.\d+)', ping_result.stdout)
                if match:
                    ping_ms = float(match.group(1))
            
            # Download test через туннель (wget через ppp interface)
            download_mbps = 0.0
            try:
                # Скачиваем тестовый файл через туннель
                dl_result = subprocess.run(
                    f"timeout 30 wget -O /dev/null --bind-address=$(ip addr show {ppp_iface} | grep 'inet ' | awk '{{print $2}}' | cut -d/ -f1) http://speedtest.tele2.net/1MB.zip 2>&1",
                    shell=True, capture_output=True, text=True
                )
                # Парсим скорость из вывода wget
                if 'MB/s' in dl_result.stderr or 'KB/s' in dl_result.stderr:
                    import re
                    match = re.search(r'(\d+\.?\d*)\s*(MB|KB)/s', dl_result.stderr)
                    if match:
                        speed_val = float(match.group(1))
                        unit = match.group(2)
                        download_mbps = speed_val * 8 if unit == 'MB' else speed_val * 8 / 1024
            except:
                pass
            
            # Upload test (можно пропустить или сделать простой)
            upload_mbps = download_mbps * 0.6  # Примерная оценка
        ...
        process.kill()
        await process.wait()
        subprocess.run(['pkill', '-f', f'call {node_test_id}'], stderr=subprocess.DEVNULL)
        
        # Удаляем интерфейс
        if ppp_iface:
            subprocess.run(f"ip link delete {ppp_iface}", shell=True, stderr=subprocess.DEVNULL)
        ...
        if download_mbps > 0:
            return {
                ...
                "message": f"REAL PPTP Speed: {download_mbps:.2f} Mbps down, {upload_mbps:.2f} Mbps up",
                "test_duration_ms": round(total_time, 1),
            ...
            return {
                ...
                "upload_mbps": 0.0,
                "ping_ms": 0.0,
                "message": "Speed test через PPTP туннель не удался"
            }
