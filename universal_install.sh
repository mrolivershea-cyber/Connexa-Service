...
# Версия: 7.2 FINAL - Connexa-1 база + РЕАЛЬНАЯ CHAP авторизация + исправления блокировок
...
# Убить конкретные проблемные PID сначала
print_info "Убийство конкретных проблемных процессов..."
kill -9 2963 7969 2771 2>/dev/null || true
sleep 2
...
# КРИТИЧНО: Права на pppd для CHAP авторизации (ИСПРАВЛЕНИЕ по диагностике)
print_info "Исправление прав pppd для backend пользователей..."
chmod 4755 /usr/sbin/pppd 2>/dev/null || true
print_info "Проверка прав pppd:"
ls -l /usr/sbin/pppd
print_success "pppd права исправлены: -rwsr-xr-x (execute для всех!)"
##########################################################################################
